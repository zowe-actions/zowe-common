...
class pax {
     ...
     * @param   paxLocalWorkspace  optional - local path to prepare pax; if not provided, default will be set
     * @param   paxRemoteWorkspace optional - remote path to pax on zOS; if not provided, default will be set
    ...
    static pack(args) {
        ...
        var paxLocalWorkspace = args['paxLocalWorkspace']
        var paxRemoteWorkspace = args['paxRemoteWorkspace']
        ...
        if (!paxLocalWorkspace){
            paxLocalWorkspace = DEFAULT_LOCAL_WORKSPACE
        }
...
`
            util.sftp(paxSSHHost,paxSSHPort,paxSSHUsername,paxSSHPassword,cmd)
...
exit 0
`
            util.ssh(paxSSHHost,paxSSHPort,paxSSHUsername,paxSSHPassword,cmd2)
...
get ${remoteWorkspaceFullPath}/${file} ${paxLocalWorkspace}`
            ...
            util.sftp(paxSSHHost,paxSSHPort,paxSSHUsername,paxSSHPassword,cmd3)
        } catch (ex1) {
            // throw error
            throw new Error(`Pack Pax package failed: ${ex1}`)
        } finally {
            if (keepTempFolder) {
                console.log(`${func}[warning] remote workspace will be left as-is without clean-up.`)
            } 
            else {
                try {
                    // run catch-all hooks
...
exit 0
`               
                    util.ssh(paxSSHHost,paxSSHPort,paxSSHUsername,paxSSHPassword,cmd4)
                } catch (ex3) {
                ...
                try {
                    ...
                    var resultCleaning = util.ssh(paxSSHHost,paxSSHPort,paxSSHUsername,paxSSHPassword,cmdCleaning)
